; 1. DFS
(defun DFS (TREE)
	; perform DFS on tree, return a single top-level list 
...
; test case
; (format t "test 1: ~d ~%" (DFS '((A (B)) C (D))))
; (format t "test 1: ~d ~%" (DFS '((A (B)) (C (D)))))

; 2. Depth-first iterative-deepening
(defun DFS-Depth (TREE depth)
	; DFS at level depth
	; return a signle top-level list of terminal nodes
	(if (null TREE) Nil
		(if (atom TREE) (cons TREE Nil)
			(if (> depth 0) 
				(append (DFS-Depth (first TREE) (- depth 1)) (DFS-Depth (rest TREE) depth))
				Nil
			)
		)
	)
...
(defun DFID (TREE maxDepth)
	...
	(cond
		((= maxDepth 0) (DFS-Depth TREE 0))
		(t (append (DFID TREE (- maxDepth 1)) (DFS-Depth TREE maxDepth)))
	)
)
; test case
; (format t "test 2: ~d ~%" (DFS-Depth '((A (B)) C (D)) 0))
; (format t "test 2: ~d ~%" (DFS-Depth '((A (B)) C (D)) 1))
; (format t "test 2: ~d ~%" (DFS-Depth '((A (B)) C (D)) 2))
; (format t "test 2: ~d ~%" (DFS-Depth '((A (B)) C (D)) 3))

; (format t "test 2: ~d ~%" (DFID '((A (B)) C (D)) 3))
; (format t "test 2: ~d ~%" (DFID '(A (B C) (D) (E (F G))) 3))
