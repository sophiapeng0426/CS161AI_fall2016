...
(defun at-least-one-color (n c k)
  (cond ((< k 1) nil) ; must have at least 1 color
    ((< k c) nil) 
    ((= k c) (list (node2var n k k)))
    (t (append (list (node2var n c k)) (at-least-one-color n (+ c 1) k)))
  ) ; end cond
)

; (format t "test at-least-one-color: ~d ~%" (at-least-one-color 1 1 5))
...
(defun combine-first (n c k)
  (cond ((= c k) nil)
    (t (cons (list (* (node2var n c k) -1) (* (node2var n k k) -1)) (combine-first n c (- k 1))))
  ); end cond
)
; (format t "test combine-first: ~d ~%" (combine-first 1 1 5))


(defun at-most-one-color (n c k)
  (cond ((or (> c k) (= c k)) nil)
    (t (append (combine-first n c k) (at-most-one-color n (+ c 1) k)))
  ) ; end cond
)
; (format t "test at-most-one-color: ~d ~%" (at-most-one-color 1 1 5))
...
; (format t "test generate-node-clauses: ~d ~%" (generate-node-clauses 2 5))
...
(defun generate-edge-clauses-helper (e c k)
  (cond ((= c k) (list (list (* (node2var (first e) k k) -1) (* (node2var (second e) k k) -1))))
    (t (cons (list (* (node2var (first e) c k) -1) (* (node2var (second e) c k) -1)) 
       (generate-edge-clauses-helper e (+ c 1) k)))
  ) ; end cond
)
; (format t "test generate-edge-clauses-helper : ~d ~%" (generate-edge-clauses-helper '(1 2) 2 5))
; (format t "test generate-edge-clauses-helper : ~d ~%" (generate-edge-clauses-helper '(1 2) 1 4))


(defun generate-edge-clauses (e k)
  (generate-edge-clauses-helper e 1 k)
)
; (format t "test generate-edge-clauses : ~d ~%" (generate-edge-clauses '(1 2) 5))
...
; (graph-coloring-to-sat "graph1.txt" "sat13.txt" 3)
; (graph-coloring-to-sat "graph1.txt" "sat14.txt" 4)
; (graph-coloring-to-sat "graph2.txt" "sat23.txt" 3)
; (graph-coloring-to-sat "graph2.txt" "sat24.txt" 4)
; (graph-coloring-to-sat "graph2.txt" "sat25.txt" 5)
; (graph-coloring-to-sat "graph2.txt" "sat26.txt" 6)
; (graph-coloring-to-sat "graph2.txt" "sat27.txt" 7)
; (graph-coloring-to-sat "graph2.txt" "sat28.txt" 8)
