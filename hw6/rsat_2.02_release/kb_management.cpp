 ...
 * - add_base_clause  : added 10/11/07 for 2.02 release
 * - add_conflict_clause
 * - remove_clause
 * - satisfied
 * - simplify_kb
...
/*
 * Added 10/11/07 for 2.02 release
 * 
 * Add clause c to the formula.
 * Clause c will be treated as a base clause
 * and it will not be deleted (unless subsumed).
 * 
 * Return -1 if the attempt failed (the clause could not be added).
 *        0 if the clause causes a contradiction.
 *        1 otherwise.
 *         
 * 
 * This function should only be called when the solver
 * is at decision level 1 (no decision). This requirement could be relaxed,
 * but it makes life much easier.
 *
 */
char add_base_clause(my_clause* c)
{
  int size = c->size;
  if(size<=0){
    rprintf("Attempting to add clause with size=%d to the formula.\n",size);
    return -1;
  }

  if(my_m->decision_level!=1){
    rprintf("Cannot add a clause at level other than 1.\n");
    return -1;
  }
  
  if(size==1){
    //simply assert this at level 1.
    return bcp(c->lits[0]);    
  }
  
  //declare the first 2 literals watched.
  add_watched_clause(c->lits[0],c);
  add_watched_clause(c->lits[1],c);

  //add c to the list of original clauses
  if(my_m->cur_cc>=my_m->original_clauses_array_len){
    //double the length
    my_m->original_clauses = (my_clause**)double_array_len(my_m->original_clauses,&(my_m->original_clauses_array_len),sizeof(my_clause*));
  }

  my_m->original_clauses[my_m->cur_cc++] = c;
  c->index = -(++my_m->cc);
...
{  
  ...
  double lg = log(num_learned_clauses)/log(2);
  int new_size = (int)lg;
