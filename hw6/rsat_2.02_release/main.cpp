...
void parse_options(int num,char** argv)
{
  ...
  for(int i=1;i<num;i++){
	...
	print_mode = 2;
      ...
      //the next arg must be a file that specifies the partial order of variables.
            
      if(i==num-1){
	printf("Expecting a partial order filename argument following -po.\n");
	exit(0);
      }
...
#if(ALLOW_PARTIAL_ORDER)
  //we must read the po file and initial variables' po scores here
  //before we call finish_up_init_manager because that is where we
  //put all variables into the heap.
  if(po_filename){
    read_po_file(po_filename);
  }
#endif
