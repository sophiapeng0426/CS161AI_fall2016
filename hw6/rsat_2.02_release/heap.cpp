...
#if(ALLOW_PARTIAL_ORDER)
  int ps1 = partial_order[v1];
  int ps2 = partial_order[v2];
...
void check_heap_property(heap* h,int var_count)
{
  ...
  for(int i=2;i<size;i++){
    //-
    double cur_score = scores[order[i]];
    double parent_score = scores[order[parent(i)]];
