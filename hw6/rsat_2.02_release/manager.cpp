...
void init_manager()
{
  ...
  m->level = (int*)calloc(_vc,sizeof(int));  //my_var-indexed
  m->status = (my_lit*)calloc(_vc,sizeof(my_lit)); //my_var-indexed
  m->reason = (my_clause**)calloc(_vc,sizeof(my_clause*)); //my_var-indexed
  m->score = (double*)calloc(_vc,sizeof(double)); //my_var-indexed
  scores = my_m->score;

#if(ALLOW_PARTIAL_ORDER)
  m->partial_order = (int*)calloc(_vc,sizeof(int));
  partial_order = my_m->partial_order;
#endif
  
  m->var_order_heap = (heap*)calloc(1,sizeof(heap));
  init_heap(m->var_order_heap,vc);
  
  m->saved = (my_lit*)calloc(_vc,sizeof(my_lit));
