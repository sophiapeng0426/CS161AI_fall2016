 ...
 * - parse_int
 * - enqueue
 * - read_po_file : added 11/9/07 for 2.02 release
 *
 */
...
#if(ALLOW_PARTIAL_ORDER)
/*
 * File format:
 * Each line has the form
 * 
 * P v1 v2 ... vn 0
 * where P is the priority (integer) of the variables on the line.
 * Note that the line is 0-terminated (to make life a bit easy).
 * A comment line starts with 'c'. 
 *
 * The higher P is, the higher the priority is.
 * Any variables that are not listed will be assumed to have
 * 0 priority (lowest).
 * 
 * 
 * Priority, once set, is meant to stay constant throughout the execution of RSat.
 * Priority is the first thing that will be used to order variables.
 * If two variables have the same priority, the solver selects the one with 
 * higher activity score (pretty much VSIDS) to assign first.
 *
 */
void read_po_file(char* fname)
{
  FILE* in;
  ...
  int* pscores = my_m->partial_order;

  char c;
  int vc = my_m->vc;
  while((c=getc(in))!=EOF){
    ungetc(c,in);    
    long unsigned int i = 0; //dummy
    read_line(line,&i,in);

    if(line[0]=='c'){continue;} //skip comment line

    char* to_read = line;
    //read the first #, it's the priority
    int priority = parse_int(&to_read);

    int v = -1;
    //read in variables until a 0 is encountered
    while((v=parse_int(&to_read))!=0){
      if(v<=0 || v>vc){
	rprintf("Invalid variable in partial order: %d\n",v);
	exit(0);
      }

      if(pscores[v]){
	rprintf("Variable %d repeated in partial order\n",v);
      }
